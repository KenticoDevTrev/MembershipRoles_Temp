...
interface WebPageMemberRoleProperties {
...
interface WebPageMemberRoleData {
    breakInheritance : boolean,
    requireAuthentication: boolean,
    memberRoleTagIds: Array<number>,
    memberRolePermissionSummary : MemberRolesPermissionSummary
...
export const WebPageItemMemberSecurityPageTemplate = ({ breakInheritance, requireAuthentication, memberRoleTagIds, availableRoles, memberRolePermissionSummary }: WebPageMemberRoleProperties) => {
    ...
    const [infoDismissed, setInfoDismissed] = useState((BrowserDataHelper.getLocalStorage(MemberRoleConstants._WebPageInfoDismissedKey) ??  "false") === "true");
    const [securitySummary, setSecuritySummary] = useState(memberRolePermissionSummary);

    const { execute: submit } = usePageCommand<WebPageMemberRoleData, WebPageMemberRoleData>(
    ...
    var dismissInfo = function() {
        BrowserDataHelper.setLocalStorage(MemberRoleConstants._WebPageInfoDismissedKey, "true", 30);
        ...
        <div>
            <Paper fullHeight>
                <Box spacing={Spacing.XXL} >
                        ...
                        <Headline size={HeadlineSize.L} spacingTop={Spacing.Micro} spacingBottom={Spacing.S}>Member Security (Web Page)</Headline>
                        {!warningDismissed && 
                            <>
